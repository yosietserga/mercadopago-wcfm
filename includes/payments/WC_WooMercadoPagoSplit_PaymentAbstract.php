...
/**
 * WC_WooMercadoPagoSplit_Payments
 */
class WC_WooMercadoPagoSplit_PaymentAbstract extends WC_Payment_Gateway
{
    ...
    /**
     * WC_WooMercadoPagoSplit_PaymentAbstract constructor.
    ...
    public function __construct()
    {
        ...
        $this->site_data = WC_WooMercadoPagoSplit_Module::get_site_data();
    ...
    public function getFormFields($label)
    {
        ...
        if (!empty($this->checkout_country)) {
            ...
            $form_fields['_mp_appid'] = $this->field_checkout_credential_appid();
            $form_fields['_mp_returnurl'] = $this->field_checkout_credential_returnurl();
            ...
            if (!empty($this->getAccessToken()) && !empty($this->getPublicKey())) {
                ...
                $form_fields[WC_WooMercadoPagoSplit_Helpers_CurrencyConverter::CONFIG_KEY] = $this->field_currency_conversion($this);
            }
        }
    ...
    public function field_checkout_steps()
    {
        $checkout_steps = array(
            'title' => sprintf(
                '<div class="mp-row">
              <h4 class="mp-title-checkout-body mp-pb-20">' . __('<b>Follow these steps to activate Mercado Pago in your store:</b>', 'woocommerce-mercadopago-split') . '</h4>
              ...
              <div class="mp-col-md-2 mp-text-center mp-pb-10">
                ...
                <p class="mp-text-steps mp-text-center mp-px-20">
                  ' . __('<b>Upload your credentials</b> depending on the country in which you are registered.', 'woocommerce-mercadopago-split') . '
                </p>
              </div>
              ...
              <div class="mp-col-md-2 mp-text-center mp-pb-10">
                ...
                <p class="mp-text-steps mp-text-center mp-px-20">
                  ' . __('<b>Approve your account</b> to be able to charge.', 'woocommerce-mercadopago-split') . '
                </p>
              </div>
              ...
              <div class="mp-col-md-2 mp-text-center mp-pb-10">
                ...
                <p class="mp-text-steps mp-text-center mp-px-20">
                  ' . __('<b>Add the basic information of your business</b> in the plugin configuration.', 'woocommerce-mercadopago-split') . '
                </p>
              </div>
              ...
              <div class="mp-col-md-2 mp-text-center mp-pb-10">
                ...
                <p class="mp-text-steps mp-text-center mp-px-20">
                  ' . __('<b>Configure the payment preferences</b> for your customers.', 'woocommerce-mercadopago-split') . '
                </p>
              </div>
              ...
              <p class="mp-text-steps mp-text-center mp-px-20">
                ' . __('<b>Go to advanced settings</b> only when you want to change the presets.', 'woocommerce-mercadopago-split') . '
              </p>
            </div>
            </div>'
            ),
    ...
    public function field_checkout_steps_link_homolog($country_link, $appliocation_id)
    {
        $checkout_steps_link_homolog = array(
            'title' => sprintf(
                __('Credentials are the keys we provide you to integrate quickly <br>and securely. You must have a %s in Mercado Pago to obtain and collect them <br>on your website. You do not need to know how to design or program to do it', 'woocommerce-mercadopago-split'),
                '<a href="https://www.mercadopago.com/' . $country_link . '/account/credentials/appliance?application_id=' . $appliocation_id . '" target="_blank">' . __('approved account', 'woocommerce-mercadopago-split') . '</a>'
            ),
            'type' => 'title',
    ...
    {
        $mp_public_key_prod = array(
            'title' => __('URL de retorno (página onde o shortcode de vinculação de contas for incluído. Informe exatamente igual quando criar o app no Mercado Pago)', 'woocommerce-mercadopago-split'),
            'type' => 'text',
            'description' => '',
    ...
    public function field_checkout_homolog_link($country_link, $appliocation_id)
    {
        $checkout_homolog_link = array(
            'title' => sprintf(
                ...
                '<a href="https://www.mercadopago.com/' . $country_link . '/account/credentials/appliance?application_id=' . $appliocation_id . '" target="_blank">' . __('Homologate account in Mercado Pago', 'woocommerce-mercadopago-split') . '</a>'
            ),
            'type' => 'title',
    ...
    public function field_category_store()
    {
        ...
        for ($i = 0; $i < count($category_store['store_categories_id']); $i++) {
            $option_category[$category_store['store_categories_id'][$i]] = __($category_store['store_categories_id'][$i], 'woocommerce-mercadopago-split');
        }
    ...
    public function field_mp_integrator_id()
    {
        ...
        $integrator_id = array(
            ...
            'description' => sprintf(
                ...
                '<a target="_blank" href="https://www.mercadopago.' . $links_mp['sufix_url'] . 'developers/' . $links_mp['translate'] . '/guides/plugins/woocommerce/preferences/#bookmark_informações_do_negócio">' . __('request it now.', 'woocommerce-mercadopago-split') .
                    '</a>'
            ),
    ...
    public function field_custom_url_ipn()
    {
        $custom_url_ipn = array(
            ...
            'description' => sprintf(
                ...
                '<a href="' . $this->getCountryLinkGuide($this->checkout_country) . 'guides/notifications/ipn/">' . __('our guides', 'woocommerce-mercadopago-split') .
                    '</a>'
            ),
            'default' => '',
    ...
    public function field_no_credentials()
    {
        $noCredentials = array(
            'title' => sprintf(
                ...
                '<a href="' . esc_url(admin_url('admin.php?page=mercado-pago-settings')) . '">' . __('Market Payment Configuration', 'woocommerce-mercadopago-split') .
                    '</a>'
            ),
            'type' => 'title'
        );
    ...
    {
        $message_support_title = __('Questions?', 'woocommerce-mercadopago-split');
    ...
    {
        $message_support_description = __('Check out the step-by-step of how to integrate the Mercado Pago Plugin for WooCommerce in our developer website.', 'woocommerce-mercadopago-split');
    ...
    {
        $message_link = __('Review documentation', 'woocommerce-mercadopago-split');
        $checkout_options_subtitle = array(
            'title' => sprintf(
                __('%s', 'woocommerce-mercadopago-split'),
    ...
    public function field_checkout_ready_title()
    {
        ...
        if ($this->isProductionMode()) {
            $message_ready_title = __('Everything ready for the takeoff of your sales?', 'woocommerce-mercadopago-split');
        } else {
            $message_ready_title = __('Everything set up? Go to your store in Sandbox mode', 'woocommerce-mercadopago-split');
        }
    ...
    {
        if ($this->isProductionMode()) {
            $message_ready_description = __('Visit your store as if you were one of your customers and check that everything is fine. If you already went to Production,<br> bring your customers and increase your sales with the best online shopping experience.', 'woocommerce-mercadopago-split');
        } else {
            $message_ready_description = __('Visit your store and simulate a payment to check that everything is fine.', 'woocommerce-mercadopago-split');
        }
    ...
    {
        if ($this->isProductionMode()) {
            $message_link = __('Visit my store', 'woocommerce-mercadopago-split');
        } else {
            $message_link = __('I want to test my sales', 'woocommerce-mercadopago-split');
        }
    ...
    public function is_available()
    {
        ...
        if (isset($w_cart)) {
            if (WC_WooMercadoPagoSplit_Module::is_subscription($w_cart->get_cart())) {
                return false;
            }
        }
    ...
    public function updateCredentialProduction()
    {
        ...
        $gateways = apply_filters('woocommerce_payment_gateways', array());
        foreach ($gateways as $gateway) {
            if (!strpos($gateway, "MercadoPago") || !class_exists($gateway)) {
                continue;
            }

            $key = 'woocommerce_' . $gateway::ID . '_settings';
            $options = get_option($key);
            if (!empty($options)) {
